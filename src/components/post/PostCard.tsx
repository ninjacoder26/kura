    ...
    <article className="group border-b border-[var(--border)] last:border-b-0 hover:bg-[var(--surface-hover)] transition-colors">
      <div className="flex">
        ...
        <div className="hidden sm:flex flex-col items-center gap-0.5 px-2 py-3">
          <button onClick={() => handleVote('up')} className={cn('p-0.5 rounded-[var(--r-xs)] transition-color', vote === 'up' ? 'text-[var(--brand-600)]' : 'text-[var(--fg4)] hover:text-[var(--brand-600)]')} aria-label="Upvote">
            <ArrowBigUp className="h-6 w-6" />
          </button>
          <span className={cn('text-xs font-bold tabular-nums', vote === 'up' && 'text-[var(--brand-600)]', vote === 'down' && 'text-red-500')}>
            {formatNumber(score)}
          </span>
          <button onClick={() => handleVote('down')} className={cn('p-0.5 rounded-[var(--r-xs)] transition-color', vote === 'down' ? 'text-red-500' : 'text-[var(--fg4)] hover:text-red-500')} aria-label="Downvote">
        ...
        <div className="flex-1 py-2.5 pr-3 min-w-0">
          ...
          <Link href={`/post/${post.id}`} className="block group/title">
            <h3 className="text-[15px] sm:text-base font-semibold text-[var(--fg)] group-hover/title:text-[var(--brand-600)] transition-colors leading-snug">{post.title}</h3>
          </Link>

          {post.body && <p className="mt-1.5 text-sm text-[var(--fg2)] line-clamp-2 sm:line-clamp-3 leading-relaxed">{post.body}</p>}

          <div className="flex items-center gap-0.5 mt-2 -ml-1 flex-wrap">
            {/* Mobile vote */}
            <div className="flex sm:hidden items-center gap-0.5 mr-1">
              <button onClick={() => handleVote('up')} className={cn('p-1 rounded transition-color', vote === 'up' ? 'text-[var(--brand-600)]' : 'text-[var(--fg4)]')}>
              ...
              <button onClick={() => handleVote('down')} className={cn('p-1 rounded transition-color', vote === 'down' ? 'text-red-500' : 'text-[var(--fg4)]')}>
                <ArrowBigDown className="h-5 w-5" />
              </button>
            </div>

            <Link href={`/post/${post.id}`} className="flex items-center gap-1 sm:gap-1.5 px-2 py-1.5 rounded-[var(--r-sm)] text-xs font-medium text-[var(--fg4)] hover:bg-[var(--bg-raised)] hover:text-[var(--fg)] transition-colors">
              <MessageSquare className="h-4 w-4" /> <span>{formatNumber(post.comment_count)}</span> <span className="hidden sm:inline">comments</span>
            </Link>
