...
export default function PostCard({ post, showCommunity = true }: PostCardProps) {
  ...
  return (
    <div className="post-card flex">
      {/* Vote sidebar */}
      <div className="flex flex-col items-center gap-0.5 px-2 py-2 bg-[var(--bg-raised)] rounded-l">
      ...
      <div className="flex-1 min-w-0 py-2 px-2">
        {/* Meta line */}
        <div className="flex items-center flex-wrap gap-x-1 text-[12px] text-[var(--fg4)] mb-1">
        ...
        <Link href={`/post/${post.id}`} className="block group/title">
          <h3 className="text-base font-medium text-[var(--fg)] group-hover/title:underline leading-snug">{post.title}</h3>
        </Link>

        {/* Body preview */}
        {post.body && (
          <div className="mt-1 text-sm text-[var(--fg3)] line-clamp-3 leading-relaxed">
            {post.body}
          </div>
        )}
